...
class DayTradingPosition():
    ...
    def __init__(self, id, security, shares, active, routing, open, longSignal, shortSignal, signalType=None,
                 signalDesc=None):
        self.Id = id
    ...
    def AppendInnerSummary(self, summary, posId):
        ...
        if self.GetLastTradedSummary() is not None:
            ...
            if summary.SummaryHierarchy==ExecutionSummary._MAIN_SUMMARY():
                raise Exception("Cannot append an INNER summary when the summary hierarchy is MAIN_SUMMARY for symbol {}".format(self.Security.Symbol))
            elif summary.SummaryHierarchy==ExecutionSummary._INNER_SUMMARY_2():
                mainSummary.AppendFirstInnerSummary(summary)
            elif summary.SummaryHierarchy==ExecutionSummary._INNER_SUMMARY_3():
                mainSummary.AppendSecondInnerSummary(summary)
        else:
            raise Exception(
                "Could not find a Main Position to append an inner position for symbol {}".format(self.Security.Symbol))
    ...
    def GetNetOpenShares(self, summaryOrder=ExecutionSummary._MAIN_SUMMARY()):
        todaySummaries = sorted(list(filter(lambda x: x.CumQty >= 0
                                                      and (
                                                                  self.RunningBacktest or x.Timestamp.date() == self.PosUpdTimestamp.date()),
        ...
        for summary in todaySummaries:  # first we calculate the traded positions
            if summaryOrder == ExecutionSummary._MAIN_SUMMARY():
                ...
                raise Exception("Could not find an inner summay of order {} for symbol {}".format(summaryOrder,
    ...
    def GetOpenSummaries(self, summaryOrder):
        ...
        if summaryOrder == ExecutionSummary._MAIN_SUMMARY():
            ...
            raise Exception(
                "Could not find an inner summay of order {} for symbol {}".format(summaryOrder, self.Security.Symbol))
    ...
    def GetLastTradedSummary(self, side=None):
        ...
        lastTradedSummaries = sorted(list(filter(lambda x: x.GetNetShares() != 0
                                                           and (
                                                                       self.RunningBacktest or x.Timestamp.date() == self.PosUpdTimestamp.date())
                                                           and (side is None or (
            x.Position.LongPositionOpened() if side == Side.Buy else x.Position.ShortPositionOpened())),
                                                 self.ExecutionSummaries.values())), key=lambda x: x.Timestamp,
                                     reverse=True)
    ...
    def EvaluateValidTimeToEnterTrade(self, candlebar, lowVolEntryThresholdModelParam, highVolEntryThresholdModelParam,
        ...
        if (self.LastNDaysStdDev is not None):
            ...
            if (
                    lowVolEntryThresholdModelParam.FloatValue is None or self.LastNDaysStdDev < lowVolEntryThresholdModelParam.FloatValue):
                if (
                        lowVolFromTimeModelParam.StringValue is not None and lowVolToTimeModelParam.StringValue is not None):
            ...
            if (
                    highVolEntryThresholdModelParam.FloatValue is None or self.LastNDaysStdDev >= highVolEntryThresholdModelParam.FloatValue):
                volEntry1 = False
                volEntry2 = False
    ...
    def CalculateCurrentDayProfits(self, marketData):
        ...
        if self.Open():
            ...
            if (lastTradedSummary.IsFirstInnerTradeOpen()):
                self.CalculateLastTradeProfit(lastTradedSummary.GetFirstInnerSummary(), firstOrderProfitsAndLosses,
                                              marketData)

            if (lastTradedSummary.IsSecondInnerTradeOpen()):
                self.CalculateLastTradeProfit(lastTradedSummary.GetSecondInnerSummary(), secondOrderProfitsAndLosses,
                                              marketData)
            ...
            if (self.SharesQuantity > 0 and self.SharesQuantity != 0 and self.OpenMarketData is not None
                ...
                self.CurrentProfitToSecurity = (self.CurrentProfitMonetary / (
                            self.SharesQuantity * self.OpenMarketData.OpeningPrice)) * 100
        else:
    ...
    def GetStatisticalParameters(self, candlebarsArr):
        ...
        pctChangeLastThreeMinSlope = self.GetSlope(barPosNow.Close if barPosNow is not None else None,
                                                   barPosMinusThree.Close if barPosMinusThree is not None else None)

        # Delta between current value and 50MMA
        deltaCurrValueAndFiftyMMov = self.GetSlope(barPosNow.Close if barPosNow is not None else None,
                                                   fiftyMinNoSkipMovAvg)
    ...
    def CalculatePercentageProfits(self, profitsAndLosses):
        ...
        if profitsAndLosses.NetShares > 0:
            ...
            netToCover = profitsAndLosses.MoneyOutflow + (
                        profitsAndLosses.PosMTM * (-1))  # if short PosMTM should be negative
            ...
            if profitsAndLosses.MoneyOutflow > 0:
                ...
                raise Exception(
                    "Could not calculate profit for closed position where money outflow is 0, for symbol {}".format(
                        self.Security.Symbol))
    ...
    def CalculateLastTradeProfit(self, lastTradedSummary, profitsAndLosses, marketData):
        ...
        if lastTradedSummary is not None and marketData.Trade is not None and lastTradedSummary.AvgPx is not None:
            ...
            if lastTradedSummary.SharesAcquired():  # LONG
                profitsAndLosses.MonetaryProfitLastTrade = (
                                                                       marketData.Trade - lastTradedSummary.AvgPx) * lastTradedSummary.GetNetShares()
                profitsAndLosses.ProfitLastTrade = ((
                                                                marketData.Trade / lastTradedSummary.AvgPx) - 1) * 100 if lastTradedSummary.AvgPx > 0 else 0
            else:  # SHORT
                profitsAndLosses.MonetaryProfitLastTrade = (
                                                                       lastTradedSummary.AvgPx - marketData.Trade) * lastTradedSummary.GetNetShares()
                profitsAndLosses.ProfitLastTrade = ((
                                                                lastTradedSummary.AvgPx / marketData.Trade) - 1) * 100 if marketData.Trade > 0 else 0
        else:
    ...
    def EvaluateGenericShortTrade(self, dailyBiasModelParam, dailySlopeModelParam, posMaximShortChangeParam,
        ...
        if ((dailyBiasModelParam.FloatValue is None or dailyBiasModelParam.FloatValue < 0)  # Daily Bias
                # Last 10 minute slope of 50 minute moving average
                and (
                        dailySlopeModelParam.FloatValue is None or statisticalParams.TenMinSkipSlope < dailySlopeModelParam.FloatValue)
                # Last 3 minute slope of 3 minute moving average
                and (
                        dailySlopeModelParam.FloatValue is None or statisticalParams.ThreeMinSkipSlope < dailySlopeModelParam.FloatValue)
                # Previous 3 to 6 minute slope of 3 minute moving average
                and (
                        dailySlopeModelParam.FloatValue is None or statisticalParams.ThreeToSixMinSkipSlope < dailySlopeModelParam.FloatValue)
                # Previous 6 to 9 minute slope of 3 minute moving average
                and (
                        dailySlopeModelParam.FloatValue is None or statisticalParams.SixToNineMinSkipSlope < dailySlopeModelParam.FloatValue)
                # Maximum change in last 3 minutes
                and (
                        posMaximShortChangeParam.FloatValue is None or statisticalParams.PctChangeLastThreeMinSlope > posMaximShortChangeParam.FloatValue)
                # Delta between current value and 50MMA
                and (
                        posMaxShortDeltaParam.FloatValue is None or statisticalParams.DeltaCurrValueAndFiftyMMov > posMaxShortDeltaParam.FloatValue)
    ...
    def EvaluateMACDRSIShortTrade(self, msNowParamA, msMinParamB, msMinParamBB, rsi30SlopeSkip5ParamC, msMaxMinParamD,
                                  msMaxMinParamDD,
                                  msNowMaxParamE, msNowParamF, msNowParamFF, rsi30SlopeSkip10ParamG,
                                  absMSMaxMinLast5ParamH,
                                  absMSMaxMinLast5ParamHH, sec5MinSlopeParamI, rsi14SlopeSkip3ExitParamV,
                                  ms3SlopeParamX,
                                  ms3SlopeParamXX, deltaDnParamXXX, broomsParamZ, broomsParamBias, macdRSISmoothedMode,
                                  absMaxMSPeriodParam,
                                  macdRsiOpenShortRule1ModelParam, macdRsiOpenShortRule2ModelParam,
        ...
        if (self.MACDIndicator.MSPrev < msNowParamA.FloatValue
                ...
                and self.MACDIndicator.MinMS < (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msMinParamB,
                                                     msMinParamBB))
        ...
        if (self.MACDIndicator.MSPrev >= msNowParamA.FloatValue
                and self.MACDIndicator.MS >= msNowParamA.FloatValue
                and self.MACDIndicator.MaxMS >= self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                        self.MACDIndicator.AbsMaxMS, msMaxMinParamD,
                                                                        msMaxMinParamDD)
                and (self.MACDIndicator.MaxMS > 0 and (
                        self.MACDIndicator.MS / self.MACDIndicator.MaxMS) < msNowMaxParamE.FloatValue)
                and self.MACDIndicator.MS < self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS,
                                                                    msNowParamF, msNowParamFF)
                and self.MinuteSmoothedRSIIndicator.GetRSIReggr(5) < (-1 * rsi30SlopeSkip5ParamC.FloatValue)
                ...
                and self.MACDIndicator.GetMSReggr(3) < (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, ms3SlopeParamX,
                                                     ms3SlopeParamXX))
        ...
        # line 4
        if (self.MACDIndicator.GetMaxAbsMSCrossover(absMaxMSPeriodParam.IntValue) < self.SmoothMACDRSIParam(
                macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, absMSMaxMinLast5ParamH, absMSMaxMinLast5ParamHH)
    ...
    def EvaluateMACDRSILongTrade(self, msNowParamA, msMinParamB, msMinParamBB, rsi30SlopeSkip5ParamC, broomsParamCC,
                                 broomsParamBias,
                                 msMaxMinParamD, msMaxMinParamDD, msNowMaxParamE, msNowParamF, msNowParamFF,
                                 rsi30SlopeSkip10ParamG,
                                 absMSMaxMinLast5ParamH, absMSMaxMinLast5ParamHH, sec5MinSlopeParamI,
                                 rsi14SlopeSkip3ExitParamV,
                                 ms3SlopeParamX, ms3SlopeParamXX, deltaUpParamYYY, macdRSISmoothedMode,
                                 absMaxMSPeriodParam,
                                 macdRsiOpenLongRule1ModelParam, macdRsiOpenLongRule2ModelParam,
        ...
        if (self.MACDIndicator.MSPrev >= msNowParamA.FloatValue and self.MACDIndicator.MS >= msNowParamA.FloatValue
                and self.MACDIndicator.MaxMS >= self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                        self.MACDIndicator.AbsMaxMS, msMinParamB,
                                                                        msMinParamBB)
        ...
        if (self.MACDIndicator.MSPrev < msNowParamA.FloatValue
                and self.MACDIndicator.MS < msNowParamA.FloatValue
                and self.MACDIndicator.MinMS < (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msMaxMinParamD,
                                                     msMaxMinParamDD))
                and (self.MACDIndicator.MinMS > 0 and (
                        self.MACDIndicator.MS / self.MACDIndicator.MinMS) < msNowMaxParamE.FloatValue)
                and self.MACDIndicator.MS >= (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msNowParamF,
                                                     msNowParamFF))
                and self.MinuteSmoothedRSIIndicator.GetRSIReggr(5) > rsi30SlopeSkip5ParamC.FloatValue
                ...
                and self.MACDIndicator.GetMSReggr(3) >= self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                                self.MACDIndicator.AbsMaxMS,
                                                                                ms3SlopeParamX, ms3SlopeParamXX)
    ...
    def EvaluateGenericLongTrade(self, dailyBiasModelParam, dailySlopeModelParam, posMaximChangeParam,
        ...
        if ((dailyBiasModelParam.FloatValue is None or dailyBiasModelParam.FloatValue >= 0)  # Daily Bias
                ...
                # Maximum change in last 3 minutes
                and (
                        posMaximChangeParam.FloatValue is None or statisticalParams.PctChangeLastThreeMinSlope < posMaximChangeParam.FloatValue)
                # Delta between current value and 50MMA
                and (
                        posMaximChangeParam.FloatValue is None or statisticalParams.DeltaCurrValueAndFiftyMMov < posMaxLongDeltaParam.FloatValue)
    ...
    def RunRuleTenShortPos(self, slFlipModelParam, changeFixedLossAtThisGainParam, gainMinTradeParamFixedLoss,
                           gainMinTradeParamFixedLoss2):
    ...
    def ConvertReducingCondToSummaryOrder(self, reducingCond):
        if (reducingCond == _SHORT_MACD_RSI_RULE_11_RED_1 or reducingCond == _LONG_MACD_RSI_RULE_11_RED_1):
            ...
            raise Exception(
                "Could not find a reducing Condigtion {} to translate for Symbol {} @ConvertReducingCondToSummaryOrder",
                reducingCond, self.Security.Symbol)

    def EvaluateReducingMACDRSIShortTrade(self, subTrade1GainLimit1, subTrade2GainLimit2, subTrade1GainLimit2,
                                          subTrade2LossLimit2, subTrade3GainLimit3, subTrade3LossLimit3):
    ...
    def EvaluateClosingMACDRSIShortTrade(self, candlebarsArr, msNowParamA, macdMaxGainParamJ, macdMaxGainParamJJ,
                                         gainMaxTradeParamJJJ,
                                         gainMaxTradeParamSDMult, gainMaxTradeParamFixedGain, macdGainNowMaxParamK,
                                         rsi30SlopeSkip5ExitParamL, rsi30Skip5ParamLX, msNowExitParamN,
                                         msNowExitParamNN, msMaxMinExitParamNBis,
                                         msMaxMinExitParamNNBis, msNowMaxMinExitParamP, msNowExitParamQ,
                                         msNowExitParamQQ,
                                         rsi30SlopeSkip10ExitParamR, msMaxMinExitParamS, msMaxMinExitParamSS,
                                         sec5MinSlopeExitParamT,
                                         gainMinStopLossExitParamU, gainMinStopLossExitParamUU, gainMinTradeParamUUU,
                                         gainMinTradeParamFixedLoss, gainMinTradeParamFixedLoss2,
                                         changeFixedLossAtThisGainParam, gainMinStopLossExitParamW,
        ...
        if (
                self.CurrentProfitLastTrade is None or self.MaxMonetaryLossCurrentTrade is None or self.MinuteSmoothedRSIIndicator.GetRSIReggr(
                5) is None
        ...
        # rule 1
        if ((self.MaxMonetaryProfitCurrentTrade / openQty) >= self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                                      self.MACDIndicator.PriceHMinusL,
                                                                                      macdMaxGainParamJ,
                                                                                      macdMaxGainParamJJ)
                and (self.MaxMonetaryProfitCurrentTrade > 0 and self.TGIndicator.HasValidValue() and ((
                                                                                                              self.CurrentProfitMonetaryLastTrade / self.MaxMonetaryProfitCurrentTrade) < self.TGIndicator.K))
                and self.MinuteSmoothedRSIIndicator.GetRSIReggr(5) > rsi30Skip5ParamLX.FloatValue
                and (self.MaxMonetaryProfitCurrentTrade / openQty) >= (
                        self.BollingerIndicator.TPSDStartOfTrade * gainMaxTradeParamJJJ.FloatValue)
                and (
                        self.MaxProfitCurrentTrade >= gainMaxTradeParamSDMult.FloatValue * self.PriceVolatilityIndicators.LastSDDDaysOpenStdDev)
        ...
        if (self.MACDIndicator.MSPrev >= msNowParamA.FloatValue
                and self.MACDIndicator.MS >= msNowParamA.FloatValue
                and self.MACDIndicator.MS >= self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS,
                                                                     msNowExitParamN, msNowExitParamNN)
        ...
        if (self.MACDIndicator.MSPrev >= msNowParamA.FloatValue
                and self.MACDIndicator.MS >= msNowParamA.FloatValue
                and (self.MaxMonetaryLossCurrentTrade / openQty) < (
                self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.PriceHMinusL, gainMinStopLossExitParamW,
                                        gainMinStopLossExitParamWW))
        ...
        if (self.MACDIndicator.MSPrev < msNowParamA.FloatValue
                and self.MACDIndicator.MS >= msNowParamA.FloatValue
                and self.MACDIndicator.MS >= (
                self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msMaxMinExitParamNBis,
                                        msMaxMinExitParamNNBis))
                and self.MinuteSmoothedRSIIndicator.GetRSIReggr(5) > rsi30SlopeSkip5ExitParamL.FloatValue
                and self.VolumeAvgIndicator.ValidateIfEnoughData()
        ...
        if (self.MACDIndicator.MSPrev < msNowParamA.FloatValue
                and self.MACDIndicator.MS < msNowParamA.FloatValue
                and self.MACDIndicator.MinMS < (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS,
                                                     msMaxMinExitParamNBis, msMaxMinExitParamNNBis))
                and (self.MACDIndicator.MinMS > 0 and (
                        (self.MACDIndicator.MS / self.MACDIndicator.MinMS) < msNowMaxMinExitParamP.FloatValue))
                and self.MACDIndicator.MS >= (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msNowExitParamQ,
                                                     msNowExitParamQQ))
                and self.MinuteSmoothedRSIIndicator.GetRSIReggr(5) > rsi30SlopeSkip5ExitParamL.FloatValue
        ...
        # rule 7
        if (
                self.MACDIndicator.GetMaxAbsMSCrossover(absMaxMSPeriodParam.IntValue) < self.SmoothMACDRSIParam(
            macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msMaxMinExitParamS, msMaxMinExitParamSS)
        ...
        if (self.MACDIndicator.GetMaxAbsMSCrossover(absMaxMSPeriodParam.IntValue) < msMaxMinExitParamS.FloatValue
                and (self.MaxMonetaryLossCurrentTrade / openQty) < (
                self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.PriceHMinusL, gainMinStopLossExitParamZ,
                                        gainMinStopLossExitParamZZ))
                and self.VolumeAvgIndicator.ValidateIfEnoughData()
        ...
        # rule 9
        if ((self.MaxMonetaryLossCurrentTrade / openQty) < (
        self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.PriceHMinusL, gainMinStopLossExitParamU,
                                gainMinStopLossExitParamUU))
                and (self.MaxMonetaryLossCurrentTrade / openQty) < (
                        -1 * self.BollingerIndicator.TPSDStartOfTrade * gainMinTradeParamUUU.FloatValue)
        ...
        ruleTenCond = self.RunRuleTenShortPos(slFlipModelParam, changeFixedLossAtThisGainParam,
                                              gainMinTradeParamFixedLoss, gainMinTradeParamFixedLoss2)
    ...
    def EvaluateClosingGenericShortTrade(self, candlebarsArr,
        ...
        if (self.MaxProfitCurrentTrade is not None
                ...
                and (
                        maxGainForClosingModelParam.FloatValue is not None and self.MaxProfitCurrentTrade >= maxGainForClosingModelParam.FloatValue * 100)
                and (pctMaxGainForClosingModelParam.FloatValue is not None and (
                        self.CurrentProfitLastTrade < pctMaxGainForClosingModelParam.FloatValue * self.MaxProfitCurrentTrade))
        ):
        ...
        if (self.CurrentProfitLastTrade is not None
                ...
                and (maxLossForClosingModelParam.FloatValue is not None and self.MaxLossCurrentTrade <= (
                        maxLossForClosingModelParam.FloatValue * 100))
        ...
        if (statisticalParams.ThreeMinSkipSlope is not None
                and (
                        pctSlopeToCloseShortModelParam.FloatValue is not None and statisticalParams.ThreeMinSkipSlope >= pctSlopeToCloseShortModelParam.FloatValue)):
            return _EXIT_SHORT_COND_5

        return None

    def RunRuleTenLongPos(self, slFlipModelParam, changeFixedLossAtThisGainParam, gainMinTradeParamFixedLoss,
                          gainMinTradeParamFixedLoss2):
        # rule 10
        if (slFlipModelParam.IntValue == 1):  # enabled
    ...
    def GetInnerTrade(self, reducingCond, side):

        openSummary1 = self.GetLastTradedSummary(side)

        if(not self.Open()):
            raise Exception("CRITICAL ERROR: Reducing a positin which is NOT OPEN for symbol {}".format(self.Security.Symbol))

        if(openSummary1 is None):
            raise Exception("CRITICAL ERROR: Reducing a posiiton which is not open for symbol {} and side {}".format(self.Security.Symbol,side))


        if reducingCond == _LONG_MACD_RSI_RULE_11_RED_1:
            return openSummary1
        elif reducingCond == _LONG_MACD_RSI_RULE_11_RED_2:
            return openSummary1.GetFirstInnerSummary()
        elif reducingCond == _LONG_MACD_RSI_RULE_11_RED_3:
            return openSummary1.GetSecondInnerSummary()
        elif reducingCond == _SHORT_MACD_RSI_RULE_11_RED_1:
            return openSummary1
        elif reducingCond == _SHORT_MACD_RSI_RULE_11_RED_2:
            return openSummary1.GetFirstInnerSummary()
        elif reducingCond == _SHORT_MACD_RSI_RULE_11_RED_3:
            return openSummary1.GetSecondInnerSummary()
        else:
            raise Exception("ERROR : reducingCond not recognized!".format(reducingCond))

    def EvaluateReducingMACDRSILongTrade(self, subTrade1GainLimit1, subTrade2GainLimit2, subTrade1GainLimit2,
                                         subTrade2LossLimit2, subTrade3GainLimit3, subTrade3LossLimit3):
        if not self.Open():  # we have to be open in order to reduce the position
            return None  # Position already opened
    ...
    def EvaluateClosingMACDRSILongTrade(self, candlebarsArr, msNowParamA, macdMaxGainParamJ, macdMaxGainParamJJ,
                                        gainMaxTradeParamJJJ,
                                        gainMaxTradeParamSDMult, gainMaxTradeParamFixedGain, macdGainNowMaxParamK,
                                        rsi30SlopeSkip5ExitParamL, rsi30Skip5ParamLX, msNowExitParamN, msNowExitParamNN,
                                        msMaxMinExitParamNBis,
                                        msMaxMinExitParamNNBis, msNowMaxMinExitParamP, msNowExitParamQ,
                                        msNowExitParamQQ,
                                        rsi30SlopeSkip10ExitParamR, msMaxMinExitParamS, msMaxMinExitParamSS,
                                        ...
                                        macdRsiCloseLongRule1ModelParam, macdRsiCloseLongRule2ModelParam,
                                        macdRsiCloseLongRule3ModelParam,
                                        macdRsiCloseLongRule4ModelParam, macdRsiCloseLongRule5ModelParam,
                                        macdRsiCloseLongRule6ModelParam,
                                        macdRsiCloseLongRule7ModelParam, macdRsiCloseLongRule8ModelParam,
                                        macdRsiCloseLongRule9ModelParam,
                                        slFlipModelParam):
        ...
        if (
                self.CurrentProfitLastTrade is None or self.MaxProfitCurrentTrade is None or self.MinuteSmoothedRSIIndicator.GetRSIReggr(
            5) is None
                or self.MACDIndicator.MSPrev is None or self.MACDIndicator.MS is None or self.MACDIndicator.MaxMS is None
        ...
        # rule 1
        if (
                (self.MaxMonetaryProfitCurrentTrade / openQty) >= self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                                          self.MACDIndicator.PriceHMinusL,
                                                                                          macdMaxGainParamJ,
                                                                                          macdMaxGainParamJJ)
                and (self.MaxMonetaryProfitCurrentTrade > 0 and self.TGIndicator.HasValidValue() and (
                ...
                self.BollingerIndicator.TPSDStartOfTrade * gainMaxTradeParamJJJ.FloatValue)
                and (
                self.MaxMonetaryProfitCurrentTrade / openQty) >= gainMaxTradeParamSDMult.FloatValue * self.PriceVolatilityIndicators.LastSDDDaysOpenStdDev
                and (
                self.MaxProfitCurrentTrade >= gainMaxTradeParamSDMult.FloatValue * self.PriceVolatilityIndicators.LastSDDDaysOpenStdDev)
                and (self.MaxProfitCurrentTrade >= gainMaxTradeParamFixedGain.FloatValue)
        ...
        if (self.MACDIndicator.MSPrev < msNowParamA.FloatValue
                and self.MACDIndicator.MS < msNowParamA.FloatValue
                and self.MACDIndicator.MS < (
                        -1 * self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS, msNowExitParamN,
                                                     msNowExitParamNN))
        ...
        if (self.MACDIndicator.MSPrev < msNowParamA.FloatValue
                and self.MACDIndicator.MS < msNowParamA.FloatValue
                and (self.MaxMonetaryLossCurrentTrade / openQty) < self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                                           self.MACDIndicator.PriceHMinusL,
                                                                                           gainMinStopLossExitParamW,
                                                                                           gainMinStopLossExitParamWW)
                and self.VolumeAvgIndicator.ValidateIfEnoughData()
        ...
        if (self.MACDIndicator.MSPrev >= msNowParamA.FloatValue
                ...
                and self.MACDIndicator.MaxMS >= self.SmoothMACDRSIParam(macdRSISmoothedMode,
                                                                        ...
                                                                        msMaxMinExitParamNBis, msMaxMinExitParamNNBis)
                and (self.MACDIndicator.MaxMS > 0 and (
                        (self.MACDIndicator.MS / self.MACDIndicator.MaxMS) < msNowMaxMinExitParamP.FloatValue))
                and self.MACDIndicator.MS < self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.AbsMaxMS,
                                                                    msNowExitParamQ, msNowExitParamQQ)
                and self.MinuteSmoothedRSIIndicator.GetRSIReggr(5) < (-1 * rsi30SlopeSkip5ExitParamL.FloatValue)
        ...
        self.SmoothMACDRSIParam(macdRSISmoothedMode, self.MACDIndicator.PriceHMinusL, gainMinStopLossExitParamU,
                                gainMinStopLossExitParamUU))
                and (self.BollingerIndicator.TPSDStartOfTrade is None or (
                        (self.MaxMonetaryLossCurrentTrade / openQty) < (
                        -1 * self.BollingerIndicator.TPSDStartOfTrade * gainMinTradeParamUUU.FloatValue)))
                # and (self.MaxMonetaryLossCurrentTrade / openQty) < gainMinTradeParamFixedLoss.FloatValue
                and (self.MaxLossCurrentTrade < gainMinTradeParamFixedLoss.FloatValue)
        ...
        # rule #10
        ruleTenExitCond = self.RunRuleTenLongPos(slFlipModelParam, changeFixedLossAtThisGainParam,
                                                 gainMinTradeParamFixedLoss, gainMinTradeParamFixedLoss2)
    ...
    def EvaluateClosingMACDRSILongTradeByTick(self, slFlipModelParam, changeFixedLossAtThisGainParam,
                                              gainMinTradeParamFixedLoss, gainMinTradeParamFixedLoss2):
    ...
    def EvaluateClosingMACDRSIShortTradeByTick(self, slFlipModelParam, changeFixedLossAtThisGainParam,
                                               gainMinTradeParamFixedLoss, gainMinTradeParamFixedLoss2):
        ...
        # rule #10
        ruleTenExitCond = self.RunRuleTenShortPos(slFlipModelParam, changeFixedLossAtThisGainParam,
                                                  gainMinTradeParamFixedLoss, gainMinTradeParamFixedLoss2)
        if (ruleTenExitCond is not None):
            return ruleTenExitCond
    ...
    def EvaluateClosingGenericLongTrade(self, candlebarsArr,
        ...
        if (statisticalParams.ThreeMinSkipSlope is not None
                and (
                        pctSlopeToCloseLongModelParam.FloatValue is not None and statisticalParams.ThreeMinSkipSlope < pctSlopeToCloseLongModelParam.FloatValue)
        ):
    ...
    def CanOpenPosition(self, candlebarArr, TAKE_GAIN_LIMIT, STOP_LOSS_LIMIT, IMPL_FLEXIBLE_STOP_LOSSES,
                        FLEXIBLE_STOP_LOSS_L1, END_OF_DAY_LIMIT,
                        LONG_SHORT_ON_CANDLE, longTrade):

        self.SoftTerminalCondEvaluated = self.EvaluateSoftTerminalCondition(candlebarArr, TAKE_GAIN_LIMIT,
                                                                            STOP_LOSS_LIMIT,
                                                                            IMPL_FLEXIBLE_STOP_LOSSES,
                                                                            FLEXIBLE_STOP_LOSS_L1)
        ...
        return (
                    not self.TerminalClose and self.SoftTerminalCondEvaluated is None and self.StrongTerminalCondEvaluated is None
    ...
    def EvaluateSoftTerminalCondition(self, candlebarsArr, takeGainLimitModelParam, stopLossLimitModelParam,
        ...
        if (takeGainLimitModelParam.FloatValue is not None and (
                self.CurrentProfitToSecurity is not None and self.CurrentProfitToSecurity > takeGainLimitModelParam.FloatValue * 100)):
        ...
        if (stopLossLimitModelParam.FloatValue is not None and (
                self.CurrentProfitToSecurity is not None and self.CurrentProfitToSecurity < stopLossLimitModelParam.FloatValue * 100)):
